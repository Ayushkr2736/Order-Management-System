...
// List all orders (joined with customer + product info)
...
// Create a new order (uses transaction to avoid overselling)
router.post('/', async (req, res) => {
  ...
  try {
    ...
    // lock the product row so no one else reads stale inventory
...
router.post('/:id/cancel', async (req, res) => {
  ...
  try {
    ...
    const allowedForCancel = ['pending', 'confirmed'];
    if (order.status === 'cancelled') {
      await client.query('ROLLBACK');
      return res.status(400).json({ error: 'Order is already cancelled' });
    } else if (!allowedForCancel.includes(order.status)) {
      await client.query('ROLLBACK');
